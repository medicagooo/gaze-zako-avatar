...
- Twelve hairstyles, eight ear choices, twenty-one hair accessories, ten facewear choices, eleven mouth choices and ten pupil styles; counts include none where applicable. Eight backgrounds. Skin presets display the current avatar in seven tones, with a custom picker.
...
- Thirteen localized thumbnail presets include the reference-sheet combinations and Lilac ribbons (lavender twin tails, cat ears, white side bows/double clips, solid violet eyes, no mouth, black background). Selecting a preset replaces current editable options; Create saves it to history. Accessory ID 20 appends the ribbon ensemble without reassigning old IDs. `PRESETS` in app.js supplies configurations; `sampleAccessory` in accessories.js supplies vector shapes for previews, history and exports.
