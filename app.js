/* Original fixed-anchor artwork. renderAvatar is shared by previews, history and both
   exporters. Layer order: background -> rear hair -> ears -> face -> front hair ->
   eyes/blush -> facewear -> hair accessory. History stores validated options only. */
'use strict';
const APP_VERSION='20260920.7';
let pigRenderSequence=0;
const TEXT = {
 zh:{title:'我的头像',edit:'装扮',hair:'发型',skin:'肤色',eyes:'眼睛',ears:'耳朵',accessory:'发饰',face:'面饰',background:'背景',generate:'生成头像',history:'生成历史',empty:'还没有生成的头像',clear:'清空历史',restore:'恢复编辑',remove:'删除',random:'随机搭配',reset:'重置',color:'颜色',hairColor:'发型颜色',skinColor:'肤色',eyeColor:'眼睛颜色',leftEye:'画面左侧眼睛',rightEye:'画面右侧眼睛',hetero:'异色瞳',side:'遮挡位置',left:'画面左侧',right:'画面右侧',custom:'自定义颜色',saved:'头像已保存到历史',duplicate:'这款头像已在历史中',restored:'已恢复，可以继续编辑',cleared:'历史已清空',storageError:'浏览器无法保存历史，请检查存储空间或隐私设置',corrupt:'部分历史数据无法读取，已保留原数据',downloadError:'下载失败，请重试',confirmClear:'确定清空全部生成历史？',cancel:'取消',confirm:'清空',download:'下载',size:'PNG 尺寸',footer:'gaze zako avatar · 头像工坊',hairNames:['短发','波波头','齐肩发','长直发','长卷发','单马尾','双马尾','双丸子头'],earNames:['无','猫耳','狐耳','兔耳','熊耳'],accessoryNames:['无','交叉发夹','星星发夹','蝴蝶结','双侧小蝴蝶结','花朵','女仆发箍','小皇冠'],faceNames:['无','圆框眼镜','椭圆框眼镜','方框眼镜','猫眼框眼镜','心形框眼镜','单眼眼罩'],backgroundNames:['黑色','白色','透明','蒂芙尼蓝 + 白点','粉色 + 白点','跨性别旗','粉色','蓝色']},
...
function eyeShape(x,y,color,style=0){
 ...
 if(style===7)out+=path('M0-42 9-10 29 0 9 10 0 42-9 10-29 0-9-10Z','#fff8d4');
 if(style===8)out+=ellipse(0,3,23,38,'none','stroke="#fff4dd" stroke-width="7"');
...
function mouthShape(style){
 ...
 shapes.push('<path d="M471 925Q505 963 538 925" '+stroke+'/><path d="M477 932 483 949 493 941Z" fill="#fff" stroke="#dba3a8" stroke-width="2"/>','<path d="M474 928Q505 952 535 928" '+stroke+'/><path d="M497 940V954Q509 974 521 953V937Z" fill="#eb9dac"/><path d="M509 943V956" stroke="#cc8294" stroke-width="2"/>','<path d="M475 947Q505 916 535 947" '+stroke+'/>');
 // Shift every mouth toward the chin without moving eyes or facewear.
...
function pigMouth(s){
 if(!s.mouthStyle)return '';
 const stroke='fill="none" stroke="#bd6878" stroke-width="7" stroke-linecap="round" stroke-linejoin="round"';
 const shapes=['','<path d="M225 735Q265 760 305 735" '+stroke+'/>','<path d="M220 730Q265 775 310 730" '+stroke+'/>','<path d="M218 734Q241 760 265 734Q289 760 312 734" '+stroke+'/>','<ellipse cx="265" cy="748" rx="14" ry="20" fill="#ad697c"/>','<path d="M220 730Q265 770 310 730Q302 792 265 792Q228 788 220 730Z" fill="#965c70"/><path d="M240 775Q265 758 290 776Q265 800 240 775Z" fill="#efabbc"/>','<path d="M240 730 275 747 242 765" '+stroke+'/>','<path d="M225 750H305" '+stroke+'/>','<path d="M222 730Q265 770 308 730" '+stroke+'/><path d="M230 737 240 765 252 750Z" fill="#fff" stroke="#dba3a8" stroke-width="2"/>','<path d="M225 735Q265 765 305 735" '+stroke+'/><path d="M255 748V770Q265 792 278 770V748Z" fill="#eb9dac"/>','<path d="M225 765Q265 728 305 765" '+stroke+'/>'];
 return shapes[s.mouthStyle]||shapes[1];
}function pigFacewear(s){if(!s.face)return '';const c=s.faceColor,lx=176,ly=489,rx=462,ry=608;let out='';if(s.face===1||s.face===2){const wide=s.face===2;out+=`<g fill="none" stroke="${c}" stroke-width="12"><ellipse cx="${lx}" cy="${ly}" rx="${wide?90:74}" ry="${wide?66:78}"/><ellipse cx="${rx}" cy="${ry}" rx="${wide?90:74}" ry="${wide?66:78}"/><path d="M${lx+(wide?90:74)} ${ly+4}Q320 545 ${rx-(wide?90:74)} ${ry+4}"/></g>`;}if(s.face===3)out+=`<g fill="none" stroke="${c}" stroke-width="12"><rect x="100" y="415" width="150" height="130" rx="22"/><rect x="390" y="540" width="150" height="130" rx="22"/><path d="M250 480Q320 510 390 575"/></g>`;if(s.face===4)out+=`<g fill="none" stroke="${c}" stroke-width="12"><path d="M100 415Q180 390 250 440Q248 545 165 540Q110 535 100 415ZM540 540Q460 515 390 565Q395 670 478 665Q530 660 540 540Z"/></g>`;if(s.face===5)out+=`<g fill="none" stroke="${c}" stroke-width="12"><path d="M176 420C120 350 62 455 176 545C290 455 232 350 176 420ZM462 540C406 470 348 575 462 665C576 575 518 470 462 540Z"/></g>`;if(s.face===6){const x=s.side==='left'?lx:rx;out+=`<path d="M80 350 650 710" stroke="${c}" stroke-width="16"/>${path(`M${x-65} ${ly-65}Q${x} ${ly-80} ${x+65} ${ly-65}L${x+60} ${ly+35}Q${x} ${ly+100} ${x-60} ${ly+35}Z`,c)}`;}return out;}
function pigAccessories(s){const c=s.accessoryColor;if(!s.accessory)return '';if(s.accessory===1)return `<g stroke="${c}" stroke-width="16" stroke-linecap="round"><path d="M95 350l65 50m-60 10 65-55M500 455l65 50m-60 10 65-55"/></g>`;if(s.accessory===2)return star(350,270,c);if([3,4,10,12,20,21,23,25].includes(s.accessory))return bow(690,380,c,.58)+([4,20,23].includes(s.accessory)?bow(70,395,c,.5):'');if(s.accessory===5)return Array.from({length:5},(_,i)=>ellipse(350+Math.cos(i*Math.PI*2/5)*28,270+Math.sin(i*Math.PI*2/5)*28,22,22,c)).join('')+ellipse(350,270,14,14,'#eac975');if(s.accessory===6||s.accessory===9)return `<path d="M80 330Q350 130 650 350" fill="none" stroke="${tone(c,-25)}" stroke-width="26"/>`;if(s.accessory===7||s.accessory===24||s.accessory===28)return path('M280 275 305 205 350 238 395 190 440 238 485 205 510 275Z',c)+path('M285 265H505V290H285Z',tone(c,-22));return '';}function pigEarClips(s){if(s.hair!==15||s.ears===0)return '';const c=s.hairColor,inner='#f2a9b6';const clip=(x,flip,shape,inside)=>`<g transform="translate(${x} 0) scale(${flip} 1)"><rect x="-70" y="335" width="140" height="28" rx="14" fill="${s.accessoryColor}"/><path d="M-45 347Q0 322 45 347" fill="none" stroke="${tone(s.accessoryColor,-30)}" stroke-width="7"/>${path(shape,c)}${inside?path(inside,inner):''}</g>`;const shapes={1:['M-41 334 0 215 41 334Z','M-27 321 0 248 27 321Z'],2:['M-49 337 0 160 49 337Z','M-32 321 0 201 32 321Z'],3:['M-36 342Q-65 180 0 145Q65 180 36 342Z','M-20 317Q-39 207 0 182Q39 207 20 317Z'],4:['M0 222A62 62 0 1 0 0 346A62 62 0 1 0 0 222Z','M0 253A30 30 0 1 0 0 313A30 30 0 1 0 0 253Z'],5:['M-38 340Q-88 236-18 199Q29 219 43 333Z','M-22 322Q-52 259-17 232Q13 243 25 321Z'],6:['M-45 338 0 190 45 338Z','M-27 322 0 233 27 322Z'],7:['M0 214A58 58 0 1 0 0 330A58 58 0 1 0 0 214Z','M0 243A28 28 0 1 0 0 299A28 28 0 1 0 0 243Z']};const [shape,inside]=shapes[s.ears]||shapes[1];return clip(210,1,shape,inside)+clip(585,-1,shape,inside);}
function renderPigAvatar(s){const lx=176,ly=489,rx=462,ry=608,pigMaskId="pig-skin-mask-"+(++pigRenderSequence);let out=background(s)+`<defs><mask id="${pigMaskId}" mask-type="alpha"><image href="pig-body.png" x="0" y="0" width="1000" height="1000" preserveAspectRatio="none"/></mask></defs><image href="pig-body.png" x="0" y="0" width="1000" height="1000" preserveAspectRatio="none"/><rect x="0" y="0" width="1000" height="1000" fill="${s.skinColor}" opacity=".55" mask="url(#${pigMaskId})"/>`;out+=ellipse(lx,ly,52,76,s.skinColor)+ellipse(rx,ry,52,76,s.skinColor)+ellipse(265,635,118,78,s.skinColor)+ellipse(86,580,78,52,s.skinColor)+ellipse(470,690,70,48,s.skinColor);out+=eyeShape(lx,ly,s.eyeColor,s.pupil)+eyeShape(rx,ry,s.hetero?s.rightEye:s.eyeColor,s.pupil);out+=ellipse(265,635,118,78,'#f27d91')+ellipse(86,580,78,52,'#f7a6b2','opacity=".75"')+ellipse(470,690,70,48,'#f7a6b2','opacity=".75"');out+=ellipse(240,635,19,32,'#7d2f3a')+ellipse(290,635,19,32,'#7d2f3a');out+=pigMouth(s)+`<g transform="rotate(17 320 550)">${pigFacewear(s)}</g>`+`<g transform="rotate(17 350 350)">${pigAccessories(s)+pigEarClips(s)+referenceAccessory(s,'front')}</g>`;return `<svg xmlns="http://www.w3.org/2000/svg" width="1000" height="1000" viewBox="0 0 1000 1000" role="img" aria-label="gaze zako pig avatar">${out}</svg>`;}function renderAvatar(s){if(s.hair===15)return renderPigAvatar(s);const h=s.hairColor,shadow=tone(h,-24),a=s.accessoryColor,f=s.faceColor;
 ...
 'M152 1000Q115 757 166 555Q230 319 490 315Q777 300 839 569Q888 795 839 1000Z',
 'M127 1000Q181 858 150 717Q119 440 364 337Q673 240 820 488Q875 647 821 805L916 1000Z',
 'M126 1000 150 655Q132 384 408 322Q766 252 854 585L900 1000Z',
...
out+=referenceAccessory(s,'front');
...
if(tab==='skin')html+=`<div class="tiles">${PALETTES.skinColor.map((color,i)=>`<button class="tile ${state.skinColor===color?'active':''}" data-color-key="skinColor" data-color="${color}" aria-pressed="${state.skinColor===color}"><div class="tile-visual">${renderAvatar({...state,skinColor:color})}</div><span class="tile-label">${TEXT[lang].skinNames[i]}</span></button>`).join('')}</div><div class="color-section"><label for="color-skinColor">${t('custom')}</label><input class="custom-color" id="color-skinColor" type="color" value="${state.skinColor}" data-custom="skinColor"><span class="color-hex">${state.skinColor.toUpperCase()}</span></div>`;
if(tab==='mouth')html+=expressionChoices('mouthStyle');
...
function checkForUpdate(){fetch(`version.json?check=${Date.now()}`,{cache:'no-store'}).then(r=>r.ok?r.json():null).then(v=>{if(v?.version&&v.version!==APP_VERSION&&!updateNoticeShown){updateNoticeShown=true;document.querySelector('#update-notice')?.showModal();}}).catch(()=>{});}
document.querySelector('#update-notice')?.addEventListener('close',()=>{if(document.querySelector('#update-notice').returnValue==='refresh')location.reload();});
checkForUpdate();setInterval(checkForUpdate,60000);document.addEventListener('visibilitychange',()=>{if(!document.hidden)checkForUpdate();});
