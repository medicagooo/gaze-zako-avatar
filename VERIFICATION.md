...
## Five additional reference presets - 2026-09-20

Added append-only hair IDs 13-14, ornament IDs 22-28, facewear ID 11 and backgrounds 8-9. Five editable presets bring the gallery to 19. `renderAvatar` shares new artwork with thumbnails, history and both exporters; `referenceFiveAccessory` separates rear ribbons/hats from front pins/circlets. `background` uses `sakuraFlower` outside the character transform. Compass eye-cover side is honored by controls, rendering and `fingerprint` deduplication. Existing IDs/default are retained.

Offline `scripts/build.cjs`, `scripts/check-expressions.cjs` and registry validation passed. Edge tested all five preset selections and actual 1024 PNG/SVG downloads, history reload/restore, separate left/right eye-cover history entries, every new-hair/accessory pair, three-language option counts, desktop 1327px and mobile 390px without horizontal page overflow or JavaScript errors. Visually inspected exported contact sheet and mobile preview; lowered the sailor cap to show its emblem more completely. Art is an editable vector interpretation rather than a pixel-identical reference copy. Local main only; no push/deployment.
